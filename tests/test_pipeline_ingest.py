...
class TestIngestSuccess:
    ...
    def test_real_chunker_is_used(self, mock_extract, mock_embed, mock_upsert):
        """Verify that token-based chunking runs, not a mock."""
    ...
    def test_empty_pages_skipped_by_chunker(self, mock_extract, mock_embed, mock_upsert):
        """Verify that empty pages are skipped."""
        mock_extract.return_value = [
    ...
    def test_chunks_have_total_chunks_metadata(self, mock_extract, mock_embed, mock_upsert):
        """Verify result contains pages and chunks counts."""
        mock_extract.return_value = [
...
class TestIngestErrors:
    ...
    @patch("src.ingestion.pipeline_v2.extract_pages")
    def test_chunking_error(self, mock_extract):
    ...
    @patch("src.ingestion.pipeline_v2.embed_chunks")
    @patch("src.ingestion.pipeline_v2.extract_pages")
    def test_embedding_error(self, mock_extract, mock_embed, mock_upsert):
        mock_extract.return_value = [{"text": "A" * 200, "page_num": 1}]
    ...
    @patch("src.ingestion.pipeline_v2.embed_chunks")
    @patch("src.ingestion.pipeline_v2.extract_pages")
    def test_storage_error(self, mock_extract, mock_embed, mock_upsert):
        mock_extract.return_value = [{"text": "A" * 200, "page_num": 1}]
